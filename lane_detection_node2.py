...
class LaneDetection(Node):
    def __init__(self):
        ...
        self.previous_error = 0.0  # Add this line to store previous error
        self.no_detection_count = 0  # Add this to track consecutive no-detections
        self.max_no_detection_uses = 1000  # Maximum number of times to use previous error
      ...
      # plotting contours and their centroids
        ...
        try:
            ...
            if len(cx_list) > 1:
                ...
                self.previous_error = error_x  # Store successful error
            ...
            # When Nothing was found
                ...
                if self.no_detection_count < self.max_no_detection_uses:
                    error_x = self.previous_error
                    self.centroid_error.data = error_x
                    self.centroid_error_publisher.publish(self.centroid_error)
                    self.no_detection_count += 1
                    self.get_logger().warn(
                        f"No lines detected (count: {self.no_detection_count}). "
                        f"Using previous error: {error_x}"
                    )
                else:
                    error_x = 0.0  # Default to center if we've used previous error too many times
                    self.centroid_error.data = error_x
                    self.centroid_error_publisher.publish(self.centroid_error)
                    self.get_logger().warn(
                        "No lines detected for too long. Defaulting to center."
                    )
